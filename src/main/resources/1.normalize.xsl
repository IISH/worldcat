<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Verwijder ongeldige marc --><!--<xsl:template match="marc:subfield[not(@code) or not(string-length(normalize-space(@code))=1)]"/>--><!--Leader correcties test string:from abcdefghijklmnop0rstuv xto   abcdefghijklmnop7rstuv0x -->
    <xls:template match="marc:leader"><!-- regel 3 -->
        ...
        <!-- concat -->
        <marc:leader><xsl:value-of select="concat( substring( text(), 1, 17 ), $leader18_new, substring( text(), 19, 4 ),
            $leader23_new, substring( text(), 24, 1 ))"/>
        </marc:leader>
    </xls:template>
    <!--Zie ook http://www.loc.gov/marc/bibliographic/bd008.html008 39 (Srce)-->
    <xls:template match="marc:controlfield[@tag='008']"><!-- step 1 -->
        ...
        <xsl:variable name="var3langcode_3">
            <xsl:choose><xsl:when test="$var3langcode_2='|||' and ( $var655a='Photo' or $var655a='Photo.' ) and
                $varmarcleader_pos7='k'"><xsl:value-of select="'zxx'"/>
            </xsl:when>
                <xsl:otherwise>
    ...
    </xls:template><!--step 041$a--><xls:template match="marc:datafield[@tag='041']">
        <xsl:copy>
    ...
    </xls:template><!--step 044$a--><xls:template match="marc:datafield[@tag='044']">
        <xsl:copy>
    ...
    </xls:template><!--step 245$a--><xls:template match="marc:datafield[@tag='245']">
        <xsl:copy>
            ...
            <xsl:choose><!-- 655a -->
                <xsl:when test="string-length($var245_245a)=0 and $var245_marcleader_pos7='k'">
                    <marc:subfield code="a"><!-- opmerking D, vierkante [] eromheen -->[<xsl:value-of
                            select="/marc:record/marc:datafield[@tag='655']/marc:subfield[@code='a']"/>]
                    </marc:subfield>
                </xsl:when>
                <!-- 245k -->
                <xsl:when test="string-length($var245_245a)=0 and $var245_marcleader_pos8='s'">
                    <marc:subfield code="a"><!-- TODO QUESTION: opmerking D, vierkante [] eromheen -->[<xsl:value-of
    ...
    </xls:template><!--step 020$a (isbn)Opmerking F, zorgen dat voor code 'a' en code 'z' werkt--><xls:template
            match="marc:datafield[@tag='020']/marc:subfield[@code='a' or @code='z']">
    ...
    </xls:template><!--invalid subfield codes, part 1, collector, designer, draughtsman, engraver, painter, photographer--><xls:template
            match="marc:datafield[@tag='100' or @tag='110' or @tag='600' or @tag='610' or @tag='648' or @tag='650' or @tag='655'or @tag='700' or @tag='710' or @tag='830']/marc:subfield[@code='collector']"><!-- TODO is het nodig om te controlen of originele waarde f is ??? -->
    ...
    </xls:template><!--replace function--><xsl:template name="string-replace-all">
        <xsl:param name="text"/>
    ...
    </xsl:template>    <xsl:template match="marc:subfield[@code='']"/></xsl:stylesheet>