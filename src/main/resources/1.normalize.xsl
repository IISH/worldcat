<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="landcodes">
        <xsl:param name="lc"/>
        <xsl:choose>
            <xsl:when test="$lc='uk#'">xxk</xsl:when>
            <xsl:when test="$lc='uk '">xxk</xsl:when>
            <xsl:when test="$lc='uk|'">xxk</xsl:when>
            <xsl:otherwise>
    ...
    <xls:template match="marc:controlfield[@tag='008']">
        ...
        <xsl:variable name="var3Postfix_new">
            <xsl:choose>
                <xsl:when test="$var3Postfix='' or $var3Postfix='  d' or $var3Postfix='| d' or $var3Postfix='||d'">d
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:variable name="var3NewPubCode">
            <xsl:choose>
                <xsl:when test="string-length($var044a)=2">
                    <xsl:value-of select="concat($var044a, '#')"/>
                </xsl:when>
                <xsl:when test="string-length($var044a)=3">
        ...
        <xsl:variable name="landcode_correctie">
            <xsl:call-template name="landcodes">
                <xsl:with-param name="lc" select="$var3NewPubCode"/>
            </xsl:call-template>
        </xsl:variable>
        ...
        <marc:controlfield tag="008">
            <xsl:value-of select="$var3step1"/> --
            <xsl:value-of select="$var3step1_pos16_18"/> --
            <xsl:value-of select="$var044a"/> ++
            <xsl:value-of select="$var3NewPubCode"/> ++
           <!-- <xsl:value-of select="$var3NewPubCode_2"/> ++-->
            <xsl:value-of select="$landcode_correctie"/> ++
        </marc:controlfield>
